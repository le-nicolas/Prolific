...
function computeHackingStats(ew, ek, hacking_titles) {
  ...
  var notHacking = function(t) { // helper function
    ...
    if(hacking_reset_counter > 10) {
      if(hacking_now) {
        ...
        h.passive = hacking_session_passive;
        if(h.passive) {
          // Keep passive productive sessions visible even with few keystrokes.
          h.intensity = Math.max(h.intensity, 0.2);
        }
  ...
  while(i<ewn && j<ekn) {
    ...
    if(popw) { // process new window event
      ...
      passive_hacking_title = passive_titles.indexOf(cur_window) > -1;
      i++;
    } else {
      ...
      if(cur_window !== '') {
        var fhere = ek[j].s;
        if(hacking_title || passive_hacking_title) {
          if(passive_hacking_title) {
            // Passive productive work (e.g., CAD) can be productive with low typing.
            hacking_reset_counter = Math.max(0, hacking_reset_counter - 1);
            hacking_counter += 1;
            if(hacking_counter > 3) {
              if(!hacking_now) {
                hacking_start = tk;
                hacking_session_passive = true;
              }
              hacking_now = true;
            }
            if(hacking_now) {
