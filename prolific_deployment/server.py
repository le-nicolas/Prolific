...
from storage import init_db, insert_coffee_event, upsert_blog_for_timestamp
...
class CustomHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if self.path == "/addcoffee":
                coffee_time = coerce_int(data.get("time"), None)
                coffee_mg = coerce_int(data.get("mg"), 100)
                if coffee_mg is None or coffee_mg <= 0:
                    self.write_text(400, "Missing or invalid caffeine amount")
                    return

                try:
                    insert_coffee_event(timestamp=coffee_time, mg=coffee_mg)
                except ValueError as exc:
                    self.write_text(429, str(exc))
                    return
