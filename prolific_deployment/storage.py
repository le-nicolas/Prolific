...
LOG_NAME_RE = re.compile(r"^(window|keyfreq|notes|blog|coffee)_(\d+)\.txt$")
LOG_PATTERNS = (
    "window_*.txt",
    "keyfreq_*.txt",
    "notes_*.txt",
    ...
    with _WRITE_LOCK:
        with _connect(db_path=db_path) as conn:
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_coffee_day_t ON coffee_events(day_t0, t);
...
def insert_coffee_event(timestamp=None, mg=100):
    ...
    mg_value = int(mg)
    if mg_value <= 0:
        raise ValueError("Caffeine amount must be greater than 0 mg")

    with _WRITE_LOCK:
        with _connect() as conn:
            row = conn.execute(
                "SELECT COUNT(1) AS n FROM coffee_events WHERE day_t0 = ?",
                (day_t0,),
            ).fetchone()
            if int(row["n"]) >= 3:
                raise ValueError("Daily coffee limit reached (3 cups)")

            conn.execute(
                "INSERT INTO coffee_events(t, day_t0, mg, source_path) VALUES(?, ?, ?, NULL)",
                (ts, day_t0, mg_value),
            )


def fetch_coffee_events(day_t0):
    day_stamp = int(day_t0)
    with _connect() as conn:
        rows = conn.execute(
            "SELECT t, mg FROM coffee_events WHERE day_t0 = ? ORDER BY t ASC, id ASC",
            (day_stamp,),
        ).fetchall()
    return [{"t": int(r["t"]), "mg": int(r["mg"])} for r in rows]
...
def list_day_timestamps():
    with _connect() as conn:
        rows = conn.execute(
            ...
            UNION
            SELECT day_t0 FROM coffee_events
            UNION
...
def _clear_imported_rows(conn, kind, source_path):
    if kind == "window":
        ...
        conn.execute("DELETE FROM coffee_events WHERE source_path = ?", (source_path,))
    elif kind == "blog":
...
def _import_legacy_event_file(conn, kind, day_t0, source_path):
    ...
    if kind == "window":
        ...
        conn.executemany(
            "INSERT INTO coffee_events(t, day_t0, mg, source_path) VALUES(?, ?, ?, ?)",
            rows,
        )
    inserted = len(rows)
