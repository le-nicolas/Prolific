...
def _normalize_coffee_events(rows, day_t0, day_t1):
    collapsed = {}
    for row in rows:
        try:
            stamp = int(row.get("t", 0))
            value = int(row.get("mg", 0))
        except (TypeError, ValueError):
            continue
        if stamp < day_t0 or stamp >= day_t1:
            continue
    ...
    return [{"t": t, "mg": collapsed[t]} for t in sorted(collapsed)]
...
def updateEvents():
    ...
    for t0 in timestamps:
        ...
        payload = {
            ...
            "coffee_events": _normalize_coffee_events(fetch_coffee_events(t0), t0, t1),
